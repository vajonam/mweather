...
static void weather_layer_set_error() {
	...
	layer_set_frame(bitmap_layer_get_layer(wld->primary_icon_layer),
			PRIMARY_ICON_ERROR_FRAME);
...
void weather_animate(void *context) {
	...
	if (weather_data->updated == 0 && weather_data->error == WEATHER_E_OK) {
		...
		weather_animation_timer = app_timer_register(WEATHER_ANIMATION_REFRESH,
...
void weather_layer_create(GRect frame, Window *window) {
	...
	large_font = fonts_load_custom_font(
			resource_get_handle(RESOURCE_ID_FUTURA_30));
...
void weather_layer_set_temperature(int16_t t, bool is_stale) {
	...
	snprintf(wld->primary_temp_str, sizeof(wld->primary_temp_str), "%i%s", t,
			is_stale ? " " : "°");
...
static bool is_night_time(int sunrise, int sunset, int utc) {
	return utc < (sunrise - CIVIL_TWILIGHT_BUFFER) || utc > (sunset
...
void weather_layer_update(WeatherData *weather_data) {
	...
	if (weather_data->error) {
		...
		bool night_time = is_night_time(weather_data->sunrise,
				weather_data->sunset, utc);
		...
		if (strcmp(weather_data->service, SERVICE_OPEN_WEATHER) == 0) {
			weather_layer_set_icon(
					open_weather_icon_for_condition(weather_data->condition,
							night_time), AREA_PRIMARY);
		} else {
			weather_layer_set_icon(
					yahoo_weather_icon_for_condition(weather_data->condition,
							night_time), AREA_PRIMARY);
		}
		...
		if (weather_data->hourly_updated != 0 && weather_data->hourly_enabled) {
			...
			snprintf(wld->h1_temp_str, sizeof(wld->h1_temp_str), "%i%s",
					weather_data->h1_temp, "°");
