...
static void appmsg_in_received(DictionaryIterator *received, void *context) {
	...
	// Configuration Update
		...
		char* service = strcmp(service_tuple->value->cstring,
				SERVICE_OPEN_WEATHER) == 0 ? SERVICE_OPEN_WEATHER
				: SERVICE_YAHOO_WEATHER;
		char
				* scale = strcmp(scale_tuple->value->cstring, SCALE_CELSIUS)
						== 0 ? SCALE_CELSIUS : SCALE_FAHRENHEIT;
	...
	// Hourly enabled
		...
		APP_LOG(APP_LOG_LEVEL_DEBUG,
				"Message with unknown keys: t=%p c=%p e=%p", temperature_tuple,
...
static void appmsg_out_failed(DictionaryIterator *failed,
